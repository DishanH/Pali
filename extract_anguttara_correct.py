"""
Correct PDF Extraction Script for Aṅguttara Nikāya
Extracts Pali text and creates JSON files for each VAGGA (chapter)
Each vagga contains multiple suttas as sections
"""
...
class AnguttaraVaggaExtractor:
    """Extract Pali text from Aṅguttara Nikāya PDF and create vagga (chapter) JSON files"""
    ...
    def detect_vaggas(self, text: str) -> List[Dict]:
        ...
        Vaggas are marked by their ending like "Mettāvaggo paṭhamo." or numbered start "1. Mettāvaggo"
        """
        ...
        # First pass: find all vagga ending markers
        for i, line in enumerate(lines):
            line_stripped = line.strip()
            # Pattern 1: vagga ending marker like "Mettāvaggo paṭhamo."
            match1 = re.match(r'^(.+vaggo)\s+(paṭhamo|dutiyo|tatiyo|catuttho|pañcamo|chaṭṭho|sattamo|aṭṭhamo|navamo|dasamo)\.$', line_stripped, re.IGNORECASE)
            # Pattern 2: peyyālaṃ ending like "Rāgapeyyālaṃ niṭṭhitaṃ."
            match2 = re.match(r'^(.+peyyālaṃ)\s+niṭṭhitaṃ\.$', line_stripped, re.IGNORECASE)
            
            if match1:
                vagga_title = match1.group(1)
                vagga_endings.append({
                    'title': vagga_title,
                    'end_line': i
                })
            elif match2:
                vagga_title = match2.group(1)
                vagga_endings.append({
                    'title': vagga_title,
                    'end_line': i
                })
        
        # Second pass: find vagga starts by looking backwards from endings
        for idx, ending in enumerate(vagga_endings):
            vagga_title = ending['title']
            end_line = ending['end_line']
            
            # Find start: look backwards for the first sutta after previous vagga
            if idx == 0:
                # First vagga starts from beginning
                start_line = 0
                # Find first sutta
                for i in range(0, end_line):
                    if re.match(r'^\d+\.\s+.+suttaṃ$', lines[i].strip(), re.IGNORECASE):
                        start_line = i - 1  # Include the vagga title line before first sutta
                        if start_line < 0:
                            start_line = 0
                        break
            else:
                # Start after previous vagga's end
                start_line = vagga_endings[idx - 1]['end_line'] + 1
                # Skip summary lines (Tassuddānaṃ) and find first sutta
                for i in range(start_line, end_line):
                    if re.match(r'^\d+\.\s+.+suttaṃ$', lines[i].strip(), re.IGNORECASE):
                        start_line = i - 1  # Include line before first sutta (might be vagga title)
                        if start_line < vagga_endings[idx - 1]['end_line']:
                            start_line = vagga_endings[idx - 1]['end_line'] + 1
                        break
            
            # Generate ID
            vagga_id = f"an{self.book_config['nipata_num']}.{len(vaggas) + 1}"
            ...
            print(f"  ✓ Found vagga {len(vaggas)}: {vagga_title} ({vagga_id})")
    ...
    def extract_suttas_from_vagga(self, vagga_text: str) -> List[Dict]:
        ...
        Suttas are marked like: "1. Mettāsuttaṃ" followed by "1. Evaṃ me sutaṃ..."
        The section number is the number after the sutta title, not the sutta number itself
        Special case: Some vaggas have sections without sutta titles (e.g., "91-116. Atha kho...")
        """
        ...
        for i in range(start_idx, len(lines)):
            ...
            # Also handle ranges like "91-116. Atha kho..."
            ...
            if section_match:
                ...
                current_sutta = {
                    "number": section_number,  # Use the actual section number from PDF
                    "pali": "",
                    "english": "",
                    "sinhala": "",
