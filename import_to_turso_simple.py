"""
Import Pali Tipitaka JSON data to Turso database (Simple version)
This script uses turso-python package for better compatibility
"""
...
class TursoImporterSimple:
    ...
    def get_stats(self):
        ...
        result = self.client.execute_query("SELECT COUNT(*) FROM baskets")
        stats["baskets"] = int(result['results'][0]['response']['result']['rows'][0][0]['value']) if result.get('results') else 0
        
        result = self.client.execute_query("SELECT COUNT(*) FROM collections")
        stats["collections"] = int(result['results'][0]['response']['result']['rows'][0][0]['value']) if result.get('results') else 0
        
        result = self.client.execute_query("SELECT COUNT(*) FROM books")
        stats["books"] = int(result['results'][0]['response']['result']['rows'][0][0]['value']) if result.get('results') else 0
        
        result = self.client.execute_query("SELECT COUNT(*) FROM chapters")
        stats["chapters"] = int(result['results'][0]['response']['result']['rows'][0][0]['value']) if result.get('results') else 0
        
        result = self.client.execute_query("SELECT COUNT(*) FROM sections")
        stats["sections"] = int(result['results'][0]['response']['result']['rows'][0][0]['value']) if result.get('results') else 0
...
def main():
    ...
    print("=" * 60)
    print("Pali Tipitaka Turso Database Importer (Simple)")
    print("=" * 60)
    ...
    try:
        # Initialize importer
        importer = TursoImporterSimple(TURSO_DB_URL, TURSO_AUTH_TOKEN)
        ...
        success = importer.import_collection("Saṃyuttanikāyo")
